...
async def get_info_validation(user_id: int, db: Session = Depends(get_db)):
    ...
    if res is None:
        raise HTTPException(
            status_code=404, detail="No validations data found")
    return res
...
async def new_info_validation(
    ...
    try:
        ...
        if res is None:
            raise HTTPException(
                status_code=404, detail="Error while creating this conso data")
        return res
...
async def validate_infos(
    ...
    try:
        ...
        info_validation = crud_validation.get_validations_by_user_id_and_field(
            db, infos.user_id, infos.validation_field)
        if info_validation is None:
            raise HTTPException(
                status_code=404, detail="There is no such code in db")
