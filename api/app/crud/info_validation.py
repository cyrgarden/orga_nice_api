...
from app.crud.utils import get_all, send_mail
from app.schemas.info_validation import InfoValidation, InfoValidationCreate, InfoValidationOrderBy
...
def create_validation(db: Session, info_validation: InfoValidationCreate):
    ...
    existing_validation = get_validations_by_user_id_and_field(db, info_validation.user_id, info_validation.validation_field)
    ...
    db_info_validation.validation_code = ''.join(random.SystemRandom().choice(string.ascii_uppercase + string.digits) for _ in range(6))


    #Generate the current timsestamp
    now = time.time()
    current_GMT = time.gmtime(now)
    timestamp = datetime.datetime(current_GMT.tm_year, current_GMT.tm_mon, current_GMT.tm_mday, current_GMT.tm_hour, current_GMT.tm_min, tzinfo=datetime.timezone.utc).timestamp()
    ...
    if info_validation.validation_field == 'mail' :
        send_mail(db, user.mail, "VALIDATION CODE", "Your code is {}".format(db_info_validation.validation_code))
...
def delete_info_validation(db: Session, validation_id: int):
    db_info_validation = db.query(models.InfoValidation).filter(models.InfoValidation.id == validation_id).first()
...
def clean_info_validation(db: Session, limit_timestamp):
    print("CLEAN CRUD")
    db.query(models.InfoValidation).filter(int(models.InfoValidation.timestamp) <= limit_timestamp).delete()
